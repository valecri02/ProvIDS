...
SAVE_DIR_BASE="${SAVE_DIR_BASE:-$PROVIDS_DIR/RESULTS/tgrab_long_range_grid}"


MEMORY="${MEMORY:-false}"

LAGS=(${LAGS:-1 4 16 32})
DISTANCES=(${DISTANCES:-1 2 4 8 16})
...
for LAG in "${LAGS[@]}"; do
  for DISTANCE in "${DISTANCES[@]}"; do
    DATA_NAME="darpa_tgrab_long_range_lag${LAG}_d${DISTANCE}"
